  ...
  <div style="width: 100%">
    <div class="submissionItem">
      <div class="raw">
        ...
        <!-- [{{ index + 1 }}] <el-link :href="paperInfo.paperLink.link" type="primary">{{ paperInfo.paperLink.name }}</el-link> -->
        [
      ...
      <div class="raw">
        <b>Subjects:</b>
        {{ paperInfo.primaryCategory.name + "," }}
      ...
      <div class="raw">
        <b>Authors:</b>
        <!-- <template v-for="(item, i) in paperInfo.authorList" :key="i">
          <el-link type="primary">{{ item + ", " }}</el-link>
        </template> -->
        <el-link type="primary">{{ paperInfo.authors }}</el-link>
      </div>
      <div class="raw">
        <div style="word-break: break-all; word-wrap: break-word">
          <b>Abstract: </b>{{ data.isExpand ? paperInfo.abstracts : capitalize(paperInfo.abstracts) }}
          <span :hidden="expandVisible" @click="expandClick()" style="color: coral">
            {{ data.isExpand ? "收起" : "展开 " }}
          </span>
        </div>
      </div>
      <div class="raw"><b>SubmitDate:</b> {{ moment(paperInfo.updateDate).format("YYYY-MM-DD") }}</div>
      <div v-if="paperInfo.comments != undefined || paperInfo.comments != null" class="raw">
        <b>Comments:</b> {{ paperInfo.comments }}
      </div>
      <div class="btn raw"></div>
    </div>
  </div>
</template>
